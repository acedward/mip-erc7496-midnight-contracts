...
The reference implementation of **[MIP-0018, "On-Chain Token Metadata
Emission"](https://github.com/midnightntwrk/midnight-improvement-proposals/pull/325)**
(MIP PR #325, `mips/mip-0018-on-chain-token-metadata.md` @ `37a3471`): a Compact
module and reference contracts that publish a token's `name`, `symbol`,
`decimals` and arbitrary key/value traits as contract events, so an indexer can
build a token table without a registry and without trusting anyone but the
emitting contract.
...
it and records how this repository implements it — the module's API, the legacy
event name still on chain, the measured circuit cost and the fixtures. This file
...
offset 33  32  key          key bytes, NUL-padded; a `/metadata/` key must be
                            a valid RFC 6901 JSON Pointer
offset 65   1  val-type     0 opaque · 1 string · 2 integer (Uint<8*val-len>,
                            little-endian, 1..31 bytes) · 3 ONE complete JSON
                            value · 4 absolute URI · 5 Null (val-len 0)
                            6..255 reserved — reject
offset 66   1  val-len      0..189
offset 67 189  value
```

**The event name is the layout version.** The reference set on Stagenet was
deployed while the MIP number was still unassigned and emits
`mip-xxxx:token-metadata[v1]`; a conforming v1 consumer *ignores* those events
and nothing was redeployed — see TOKEN-METADATA.md, "The legacy name".

Status: **experimental**, tracking a Proposed MIP.
...
fixtures/simulator/                  the offline corpus, regenerated by that script
fixtures/legacy-mip-xxxx/            the frozen `mip-xxxx` corpus of the #315 draft
fixtures/stagenet/                   the on-chain record of the Stagenet deployment
...
allows. Widening `decimals` from one byte to sixteen (`Uint<128>`) moves nothing:
it is a literal in the deployed contracts and a `Bytes<189>` field in the
parameterised ones either way. ZKIR v3 measured ~4.4× cheaper across the board.
...
the current `mip-0018:token-metadata[v1]` implementation above, whose standard
fields use `Bytes<32>` symbols and a `Uint<128>` `decimals`. Their pinned
`compact-src/` snapshots are reproducibility artefacts and are deliberately left
at their audited revision.
...
sections 1, 2.1, 2.2, 3 and 5.1 from the MIP text rather than importing anything
from the contracts, so a disagreement fails a test.
...
complete value, a relative URI, a Null with a non-zero length, or a `/metadata/`
key that is not a valid RFC 6901 pointer.

Two rows carry the rules the final MIP text added. `SNEB` publishes its
`metadata` as **one** complete 141-byte JSON value — it used to publish a
six-part `metadata/<n>` document, which MIP-0018 rejects because a fragment is
not one complete JSON value and section 5.4 defines no reassembly — plus one
`/metadata/description` pointer key to show that nothing about arrays or nesting
follows from the path. `LMOON` renames itself in a later block and then
**clears** its `description` with a Null (val-type 5, val-len 0), so the corpus
holds both the history and the cleared current value.

`deployments/generated-matrix.json` is regenerated with the contracts and
describes what a *new* deployment would do. The record of the deployment that
actually happened is `deployments/stagenet-deployment.json`.
...
| file | what is in it |
|---|---|
| `events.json` | all 66 `mip-0018:token-metadata[v1]` events, with their 256-byte payloads as hex |
...
| `negative-payloads.json` | 32 payloads that are not simply applied: 14 a consumer must **reject** (one per rule of MIP sections 2.1, 2.2, 3 and 5.1, each with its reason), 2 it must **ignore** (the pre-MIP `TokenMetadata` name and the `mip-xxxx` placeholder), and 16 it must **apply** — six of them well-known keys whose Appendix A projection fails while the event stands |

Every rule the final text added has a payload on **each** side of it: a Null with
`val-len` 0 (applied) and with `val-len` 4 (rejected); val-type **6** as the
first reserved value; a 3-byte integer (applied, and it still projects to 6 —
`Uint<128>` is an emitter default, not a decoder fallback) and a 32-byte one
(rejected); a JSON scalar (applied) beside the old `metadata/0` fragment
(rejected); `/metadata/0` and `/metadata/a~1b~0c` (applied) beside
`/metadata/~2` (rejected, `key_pointer_invalid`).
...
### `fixtures/legacy-mip-xxxx/` — the frozen draft corpus

The same five files as `fixtures/simulator/`, as they were produced against the
**#315 draft** under the `mip-xxxx:token-metadata[v1]` placeholder name. They are
never regenerated and nothing in this repository's tests reads them; they exist
because the Stagenet deployment still emits that name and a consumer that
deliberately keeps a legacy path needs a corpus to pin. `SOURCE.md` there
tabulates every rule that differs. The current corpus also carries the
placeholder name as an `ignored` payload, so the *conforming* behaviour can be
pinned without this directory.

### `fixtures/stagenet/` and `deployments/stagenet-deployment.json` — the live record

Both are recorded from the **Stagenet redeployment of 2026-09-18**, which emits
the pre-final `mip-xxxx:token-metadata[v1]` name (see TOKEN-METADATA.md, "The
legacy name"): eleven contracts, 48
...
Because the name moved and nothing was redeployed, `contracts/generated/` and
`contracts/managed/` here are the MIP-0018 sources, not the sources of those
addresses — those are at
